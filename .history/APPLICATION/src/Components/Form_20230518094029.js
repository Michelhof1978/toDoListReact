...
La première partie du code est consacrée à la création de l'état de l'application à l'aide du hook useState.

dataArray est le tableau qui contient les différentes tâches à faire. Chaque tâche est un objet contenant un texte (txt) et un identifiant unique (id) généré par la fonction uuidv4.

newStateInput est un autre état qui contient la valeur de l'input du formulaire. Il est initialisé comme vide.

Fonction supElement :

Cette fonction est utilisée pour supprimer une tâche. Elle prend en paramètre un id et filtre le tableau dataArray pour créer un nouveau tableau sans la tâche ayant cet identifiant. Ensuite, elle met à jour dataArray avec ce nouveau tableau.

Fonction addTodoList :

Cette fonction est déclenchée lorsque l'utilisateur soumet le formulaire. Elle crée un nouvel objet newTodoList avec le texte entré par l'utilisateur et un nouvel identifiant unique, puis l'ajoute au tableau dataArray. Ensuite, elle réinitialise newStateInput à vide.
