...
export default function Form(){
    ...
    const addTodo = e => {
        ...
        const newArr = [...dataArray] //On a creé un nouveau tableau avec tous les objects à l intérieur car, on ne peut pas recopier un tableau ou un object existant donc on va toujours utiliser le spread operator qui va récuperer tous les éléments de dataArr
        ...
        setDataArray(newArr);//On change le state avec les nouvelles données que l on aura donné ds le champ de formulaire
...
<form onSubmit={e => addTodo(e)} className="mb-3">
            ...
            <ul className="list-group"> {/*On retourne une liste avec les Items grâce à la classe list-group de bootstrap */}
                {dataArra.map(item => {// map va retourner un nouveau tableau ,item est un paramètre de la fonction de rappel utilisée dans la méthode map()
                    return (
